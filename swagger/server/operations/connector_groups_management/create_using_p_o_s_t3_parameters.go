/* #nosec */ // Code generated by go-swagger; DO NOT EDIT.

package connector_groups_management

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"net/http"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	"github.com/go-openapi/runtime/middleware"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/validate"
)

// NewCreateUsingPOST3Params creates a new CreateUsingPOST3Params object
// no default values defined in spec.
func NewCreateUsingPOST3Params() CreateUsingPOST3Params {

	return CreateUsingPOST3Params{}
}

// CreateUsingPOST3Params contains all the bound params for the create using p o s t 3 operation
// typically these are obtained from a http.Request
//
// swagger:parameters createUsingPOST_3
type CreateUsingPOST3Params struct {

	// HTTP Request Object
	HTTPRequest *http.Request `json:"-"`

	/*Name of Group
	  Required: true
	  In: query
	*/
	GroupName string
}

// BindRequest both binds and validates a request, it assumes that complex things implement a Validatable(strfmt.Registry) error interface
// for simple values it will use straight method calls.
//
// To ensure default values, the struct must have been initialized with NewCreateUsingPOST3Params() beforehand.
func (o *CreateUsingPOST3Params) BindRequest(r *http.Request, route *middleware.MatchedRoute) error {
	var res []error

	o.HTTPRequest = r

	qs := runtime.Values(r.URL.Query())

	qGroupName, qhkGroupName, _ := qs.GetOK("groupName")
	if err := o.bindGroupName(qGroupName, qhkGroupName, route.Formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

// bindGroupName binds and validates parameter GroupName from query.
func (o *CreateUsingPOST3Params) bindGroupName(rawData []string, hasKey bool, formats strfmt.Registry) error {
	if !hasKey {
		return errors.Required("groupName", "query", rawData)
	}
	var raw string
	if len(rawData) > 0 {
		raw = rawData[len(rawData)-1]
	}

	// Required: true
	// AllowEmptyValue: false
	if err := validate.RequiredString("groupName", "query", raw); err != nil {
		return err
	}

	o.GroupName = raw

	return nil
}
